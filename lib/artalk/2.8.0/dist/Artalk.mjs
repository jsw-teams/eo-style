...
function renderReplyTo(r) {
  ...
  if (r.cConf.replyTo.is_collapsed)
    replyContent = `[${r.ctx.$t("collapsed")}]`;
...
class CommentActions {
  ...
  vote(type) {
    ...
    this.ctx.getApi().votes.vote(`comment_${type}`, this.data.id, __spreadValues({}, this.ctx.getApi().getUserFields())).then((res) => {
      var _a, _b;
  ...
  adminEdit(type, btnElem) {
    ...
    this.ctx.getApi().comments.updateComment(this.data.id, __spreadValues({}, modify)).then((res) => {
  ...
  adminDelete(btnElem) {
    ...
    this.ctx.getApi().comments.deleteComment(this.data.id).then(() => {
      btnElem.setLoading(false);
...
function createComment(ctx, comment, ctxComments) {
  var _a;
  const instance2 = new CommentNode(ctx, comment, {
