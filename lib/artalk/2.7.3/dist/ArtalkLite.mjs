...
class UserApi extends ApiBase {
  ...
  loginStatus() {
    return __async(this, null, function* () {
      const data = yield this.POST("/login-status", this.withUserInfo({}));
...
function convertApiOptions(conf, ctx) {
  return {
    ...
    apiToken: ctx == null ? void 0 : ctx.get("user").getData().token,
...
function showErrorDialog(opts) {
  ...
  if (opts.onOpenSidebar) {
    const $openSidebar = createElement(`<span atk-only-admin-show> | <span style="cursor:pointer;">${t("open") + t("ctrlCenter")}</span></span>`);
