...
var nullthrow = require('foxact/nullthrow');
...
function useConfig() {
    return nullthrow.nullthrow(react.useContext(ConfigContext), '<ConfigProvider /> is missing');
}

function DisqusJSPostItem({ comment, children, nesting }) {
    const { admin, adminLabel, disqusJsModeAssetsUrlTransformer = identity.identity } = useConfig();
...
function DisqusJSChildrenPostItems({ children, nesting: currentNesting = 1 }) {
    ...
    return /*#__PURE__*/ jsxRuntime.jsx("ul", {
        ...
        children: children.map((comment)=>/*#__PURE__*/ jsxRuntime.jsx(DisqusJSPostItem, _object_spread({}, comment), comment.comment.id))
    });
}
...
function DisqusJSCommentsList({ comments }) {
    ...
    return /*#__PURE__*/ jsxRuntime.jsx("ul", {
        ...
        children: processedComments.map((comment)=>/*#__PURE__*/ jsxRuntime.jsx(DisqusJSPostItem, _object_spread({}, comment), comment.comment.id))
    });
}
...
function DisqusJSPosts({ id }) {
    ...
    const fetchMorePosts = react.useCallback((reset = false)=>_async_to_generator(function*() {
            if (!id) return;
...
function DisqusJSThread() {
    ...
    const fetchThread = react.useCallback(()=>_async_to_generator(function*() {
            try {
