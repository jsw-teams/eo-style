...
const useStore = create__default["default"]((set, get)=>({
        ...
        async fetchMorePosts (shortname, id, apiKeys, api = 'https://disqus.com/api/') {
            ...
            const url = posts.length === 0 ? `${api}3.0/threads/listPostsThreaded?forum=${shortname}&thread=${id}&order=${sortType ?? 'desc'}` : `${api}3.0/threads/listPostsThreaded?forum=${shortname}&thread=${id}${lastPost?.cursor.next ? `&cursor=${lastPost.cursor.next}` : ''}&order=${sortType ?? 'desc'}`;
            const handleError = ()=>{
                if (posts.length === 0) {
                    set({
                        error: true,
...
const DisqusJSLoadMoreCommentsButton = /*#__PURE__*/ react.memo((props)=>{
...
function DisqusJSChildrenPostItems(props) {
    ...
    return /*#__PURE__*/ jsxRuntime.jsx("ul", {
        ...
        children: props.children.map((comment)=>/*#__PURE__*/ react.createElement(DisqusJSPostItem, {
                ...comment,
                admin: props.admin,
...
const DisqusJSPosts = (props)=>{
    ...
    const fetchFirstPageRef = react.useRef(false);
    ...
    if (posts.length > 0) {
        return /*#__PURE__*/ jsxRuntime.jsxs(jsxRuntime.Fragment, {
            children: [
                ...
                posts.at(-1)?.cursor.hasNext && /*#__PURE__*/ jsxRuntime.jsx(DisqusJSLoadMoreCommentsButton, {
...
const DisqusJSThread = (props)=>{
    ...
    const fetchThreadRef = react.useRef(false);
    react.useEffect(()=>{
...
const DisqusJSEntry = (props)=>{
    ...
    if (disqusJsMode === 'dsqjs') {
        return /*#__PURE__*/ jsxRuntime.jsx(DisqusJSThread, {
            ...props
        });
    }
    return null;
};
const DisqusJS = /*#__PURE__*/ react.forwardRef((props, ref)=>{
