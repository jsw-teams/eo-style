...
(function (window) {
  ...
  bt._isObject = function (source) {
    return 'function' === typeof source || !!(source && 'object' === _typeof(source));
  }; //解析模板字符串
  ...
  bt._analysisStr = function (str) {
    ...
    str = String(str) //去掉分隔符中js注释
    .replace(new RegExp("(" + _left + "[^" + _right + "]*)//.*\n", "g"), "$1") //去掉注释内容  <%* 这里可以任意的注释 *%>
    //默认支持HTML注释，将HTML注释匹配掉的原因是用户有可能用 <! !>来做分割符
    .replace(new RegExp("<!--.*?-->", "g"), "").replace(new RegExp(_left + "\\*.*?\\*" + _right, "g"), "") //把所有换行去掉  \r回车符 \t制表符 \n换行符
    ...
    str = str //定义变量，如果没有分号，需要容错  <%var val='test'%>
...
})(window); // Import baiduTemplate finished
...
var setLS = function setLS(key, value) {
  try {
...
var checkDisqus = function checkDisqus() {
  var img = new Image();
  var check1 = setTimeout(function () {
...
var forceDsqjs = function forceDsqjs() {
  setLS('disqusjs_mode', 'dsqjs');
  main();
};

var forceDisqus = function forceDisqus() {
  setLS('disqusjs_mode', 'disqus');
  main();
};
/*
...
var getThreadInfo = function getThreadInfo() {
...
var getCommentList = function getCommentList(data) {
  ...
  function getChildren(id) {
    ...
    for (var _i = 0; _i < childComments.length; _i++) {
      var comment = childComments[_i];
...
var renderComment = function renderComment(data) {
  /*
  ...
  */
  ...
  data.map(function (s) {
    var childrenComments = function childrenComments(s) {
      ...
      children.map(function (s) {
        ...
        if (comment.author.profileUrl) {
          ...
          */
          comment.avatarEl = "<a href=\"".concat(comment.author.profileUrl, "\" target=\"_blank\" rel=\"nofollow noopener noreferrer\"><img src=\"").concat(comment.author.avatar.cache, "\"></a>");
          comment.authorEl = "<a href=\"".concat(comment.author.profileUrl, "\">").concat(comment.author.name, "</a>");
        } else {
          comment.avatarEl = "<img src=\"".concat(comment.author.avatar.cache, "\">");
          comment.authorEl = "".concat(comment.author.name);
        }

        if (comment.author.name === disqusjs.config.admin) {
          comment.authorEl += "<span class=\"dsqjs-admin-badge\">".concat(disqusjs.config.adminLabel, "</span>");
        }

        s.nesting = nesting + 1;
        html += "<li class=\"dsqjs-item\" id=\"comment-".concat(comment.id, "\">");
        html += baidu.template(commentBodyTpl, comment);
        html += "".concat(childrenComments(s), "</li>");
      });
      html += '</ul>';
    ...
    var html = "<li class=\"dsqjs-item\" id=\"comment-".concat(comment.id, "\">");
    html += baidu.template(commentBodyTpl, comment);
    html += "".concat(childrenComments(s), "</li>");
